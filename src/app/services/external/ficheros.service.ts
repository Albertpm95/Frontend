...
export class FicheroService {
  ...
  obtenerFicheros(): Observable<FicheroSubido[]> {
    return this.http.get<FicheroSubido[]>(`${environment.apiUrl + endpoints.utils.files.list}`);
  }

  enviarListaFicherosProcesar(listaFicherosServidorSeleccionados: FicheroSubido[]) {
  ...
  ejecutarLinearRegresion() {
    return this.http.get(`${environment.apiUrl + endpoints.linear_regresion.uploadYEjecutarLinearRegresion}`);
  }
  ejecutarNNLinearRegresion() {
    return this.http.get(
      `${environment.apiUrl + endpoints.non_negative_linear_regression.non_negative_linear_regression}`,
    );
  }
}
