...
@Component({
  ...
  template: `
    <div>
      <span class="text-xl font-semibold">Lista de ficheros en el servidor</span>

      <ul class="mt-5">
        @for (fichero of store.listaFicherosServidorSeleccionados(); track fichero.id) {
          <li
...
export class FicherosServidorSeleccionadosComponent {
  ...
  enviarListaFicherosProcesar() {
    const listaFicherosServidorSeleccionados = this.store.listaFicherosServidorSeleccionados();
    if (listaFicherosServidorSeleccionados.length === 2)
      this.#ficherosService.enviarListaFicherosProcesar(listaFicherosServidorSeleccionados).subscribe((event: any) => {
        switch (event.type) {
